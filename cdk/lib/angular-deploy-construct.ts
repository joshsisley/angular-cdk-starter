...
export class SPADeploy extends Construct {
  ...
  private getS3Bucket(config: SPADeployConfig, isForCloudFront: boolean) {
    ...
    if (
      this.globalConfig.ipFilter === true ||
      isForCloudFront === true ||
      typeof config.blockPublicAccess !== "undefined"
    ) {
  ...
  private getCFConfig(
    ...
    const cfConfig: any = {
      ...
      errorConfigurations: [
        {
          errorCode: 403,
          responsePagePath: config.errorDoc
            ? `/${config.errorDoc}`
            : `/${config.indexDoc}`,
          responseCode: 200,
        },
    ...
    if (
      ...
      typeof config.cfAliases !== "undefined"
    ) {
  ...
  public createBasicSite(config: SPADeployConfig): SPADeployment {
    ...
    if (config.exportWebsiteUrlOutput === true) {
      if (
        typeof config.exportWebsiteUrlName === "undefined" ||
        config.exportWebsiteUrlName === ""
      ) {
        throw new Error(
          "When Output URL as AWS Export property is true then the output name is required"
        );
      }
   ...
   */
  public createSiteWithCloudfront(
    ...
    const distribution = new CloudFrontWebDistribution(
      ...
      this.getCFConfig(websiteBucket, config, accessIdentity)
    );
  ...
  public createSiteFromHostedZone(
    ...
    const domainName = config.subdomain
      ? `${config.subdomain}.${config.zoneName}`
      : config.zoneName;
    ...
    const distribution = new CloudFrontWebDistribution(
      this,
      "cloudfrontDistribution",
      this.getCFConfig(websiteBucket, config, accessIdentity, cert)
    );
